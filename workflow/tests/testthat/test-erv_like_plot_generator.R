...
#
# The panel now reads the genus-founded loci table produced by taxonomy_classify
...
# load_genus_loci coerces the string-typed structural columns.
#
...
# A minimal genus-founded loci frame, mirroring the classifier's LOCI_COLUMNS
# (structural columns arrive as strings from parquet; load_genus_loci coerces).
.loci <- function(n = 2L) {
  tibble::tibble(
    ...
    genus_call = c("Gammaretrovirus", "Betaretrovirus")[seq_len(n)],
    rank = "genus", confidence = 1, method = "lca",
...
test_that("builders fall back to empty_plot on zero-row input", {
  ...
  expect_match(composition_heatmap_plot(el)$labels$title, "no genus-resolved loci")
})

# load_genus_loci coerces the classifier's string-typed structural columns.
test_that("load_genus_loci coerces structural columns + derives span_bp", {
  ...
  df <- tibble::tibble(
    ...
    genus_call = "Gammaretrovirus", rank = "genus", confidence = "1.000",
