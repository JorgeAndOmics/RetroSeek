...
source(file.path(.resolve_script_dir(), "utils", "log.R"))  # line contract, run_main (ADR-021)
...
log_job(args$log, "circle_plot_generator")
...
# main(): sections 3 onwards, run through run_main() so warnings are logged and
...
main <- function() {
  ...
  log_info("generating circle plots for %s", tools::file_path_sans_ext(basename(args.gff_custom_file)))
  ...
  log_ok("wrote %s.png and .pdf, %s sequences", basename(args.output_file),
         format(length(common_levels), big.mark = ","))
}
