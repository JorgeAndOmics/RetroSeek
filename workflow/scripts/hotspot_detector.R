...
main <- function(args) {
  ...
  log_info("%d group(s): %s", length(gff_groups), paste(names(gff_groups), collapse = ", "))
  ...
  for (label in names(gff_groups)) {
    log_info("%s: %d hits", label, length(gff_groups[[label]]))
  ...
  log_info("total windows: %d, total hotspots: %d", nrow(all_windows_df), length(all_hotspots))
  ...
  log_ok("%s hotspots in %s windows, from %s events",
         format(length(all_hotspots), big.mark = ","),
         format(nrow(all_windows_df), big.mark = ","), format(length(hits), big.mark = ","))
}
