...
- **P0 - Setup & tracking.** [OK] Files in place; environment + dataset inventoried.
- **P1 - Reference package.** [OK] Independent genus-balanced RefSeq reference (151 proteins,
  `data/taxonomy_reference/`); spuma gap noted.
- **P2 - Classifier prototype.** [OK] `taxonomy_lca.py` (+`weighted_lca`), `taxonomy_classify_loci.py`,
  `taxonomy_classify_genome.py`, `taxonomy_reference_builder.py`, `tblastn_to_loci.py`; 13 unit tests.
- **P3 - 3 bats.** [OK] Desmodus + Antrozous (Mode A, valid loci) + Molossus (Mode B). Gamma-dominant.
- **P4 - Human/mouse.** [OK] Mode-B genome composition. Human Class I-leaning, mouse Class II-leaning.
- **P5 - Outputs.** [OK] Per-locus/per-genome CSVs in `results/`; cross-species summary in 04_FINDINGS.
- **P6 - Verification & write-up.** [OK] Literature verified (named sources); LOO 98.6%; reliability
