...
**Solo-LTR detection (native, ADR-017)** - `solo_bait_builder`, `solo_blaster`, `solo_finder`, `solo_annotator`, `solo_tree`, `solo_plot_generator`, `solo_ltr_native` (aggregate). Uses the LTR arms of ERV-bearing elements as `blastn` bait, then subtracts the hits that overlap an intact element (flanking arms) or sit within a proviral distance of an orphan locus (monoLTRs beside surviving coding sequence). What remains is a solo LTR. Writes `tracks/solo_ltr_native/` and `tables/solo_ltr_native/`. See [`docs/solo_ltr.md`](solo_ltr.md) for the mechanism and [ADR-017](adr/ADR-017-native-solo-ltr-detection.md) for the decision and its calibration.
...
- `results/tracks/` - GFF3 per stage (`original`, `candidate`, `orphans`, `valid`, `flanking_ltr`, `ltrdigest`, `ltrharvest`, `solo_ltr_native`, `taxonomy`). `orphans/{genome}.gff3` is the orphan (non-LTR) hit set, carrying a synthetic `Parent=` from proximity clustering; `orphans/{genome}.classified.{gff3,bed}` is the recovered, taxonomically-called subset. Only `valid` carries a `_reduced` variant (merged overlapping ranges); `original`/`candidate` are unreduced. The composite ERV "assembly" is no longer a separate `erv_like` track - it is now the taxon-founded `taxonomy/{genome}.gff3` (one feature per LTR-element locus, labelled by `taxon_call`). `solo_ltr_native/{genome}.gff3` carries solo LTRs with taxonomy inherited from the element whose LTR arm caught them (see `docs/solo_ltr.md`).
