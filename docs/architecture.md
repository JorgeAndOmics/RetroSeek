...
**Solo-LTR detection** - `ltr_scn_from_gff3`, `ltr_retriever_prefilter`, `ltr_retriever`, `solo_ltr_integrator`, `solo_ltr_detector` (aggregate). Reconstructs the LTRharvest SCN from its GFF3 (avoiding a suffix-array rebuild, which would cascade LTRharvest and LTRdigest across every genome), pre-filters it by `valid_ranges.gff3`, runs LTR_retriever with whole-genome annotation, and calls solos from the RepeatMasker table via the tool's own `solo_finder.pl`. Each solo inherits `taxon_call` from the classified locus its library entry came from. See [`docs/solo_ltr.md`](solo_ltr.md), [ADR-003](adr/ADR-003-ltr-retriever-pre-filter.md) and [ADR-013](adr/ADR-013-solo-ltrs-on-the-assembled-catalog.md).
...
- `data/ltr_scn/` - LTRharvest screen-format intermediates: `{genome}.scn` (raw LTRharvest stdout, when present), `{genome}_from_gff3.scn` (reconstructed from the LTRharvest GFF3 and byte-identical to a real SCN - this is what the solo-LTR chain consumes), plus `{genome}_retroviral.scn` (Coupling-A filtered) and `{genome}_full.scn` (byte-equal passthrough). The prefilter rule emits both filtered files from one read pass; one of them feeds LTR_retriever.
...
- ADR-013: solo LTRs on the assembled catalog - SCN reconstruction from the GFF3, solos taken from the RepeatMasker annotation rather than `nmtf.pass.list`, taxonomy inherited by coordinate, and the `solo-ltr` catalog tier.
