...
**Taxonomic classification** — `taxonomy_reference` + `taxonomy_reference_trees` (build-once reference: genus-comprehensive proteins via Entrez, NCBI taxonomy, curated `erv_class.tsv`, per-gene placement trees; `make reference` / `--build-reference`), then `taxonomy_classify` (per-locus genus calls), `taxonomy_fragments` (recovered fragments tier), `taxonomy_plot_generator`, and `loss_analysis` (`--classify`). Reclassifies each valid LTR-element locus from its own marker sequence — POL/GAG by phylogenetic placement, weighted-LCA otherwise — into a calibrated `genus_call` (rank, confidence, `confidence_tag` HC/LC vs `classification.confidence_min`, `n_blastx_hits`, method, mosaic, ERV class). The per-locus loci table is the genus-founded ERV assembly; legacy probe `virus`/`label` are kept as detection provenance. A parallel **fragments tier** recovers non-LTR-associated hits — reduced BLAST hits overlapping no retrotransposon — by running the same classifier on them and keeping only those that earn a taxonomic call (the novel-retrovirus path; `source=fragment`). `loss_analysis` then unions the ranges-analysis and blastx-stage counts into one per-stage attrition funnel and exports per-genome novel candidates (loci with `n_blastx_hits == 0`). See [configuration.md `## classification`](configuration.md), [`docs/taxonomy_classification/`](taxonomy_classification/), and [ADR-007](adr/ADR-007-taxonomic-classification.md).
**Downstream analyses** — `plot_generator` (phase-modular: `workflow/scripts/plot2sort.R` is a slim orchestrator over sibling modules in `workflow/scripts/plot2sort/{helpers,io,plots_distribution,plots_categorical,plots_sankey}.R`; emits 21 PNGs with auto-scaled canvases driven by `auto_dims()`, opt-in long-tail collapse via `sankey_top_n`, and waffle auto-scaling), `stage_plot_generator` (18 middle-stage PNGs: concordance/structure/funnel/multiplicity + pre-reduction overlap + LTR-interaction), `erv_like_plot_generator` (6 ERV-like *structural* PNGs read from the genus-founded taxonomy loci table: completeness, canonical order, gene combinations, length, main-gene count, genus×gene composition), `circle_plot_generator`, `hotspot_detector`, `pair_detector`. `--generate-global-plots` drives the provirus panel (`plot_generator` + `stage_plot_generator`) plus the erv-like panel (`erv_like_plot_generator`).

## Outputs

- `results/tracks/` — GFF3 per stage (`original`, `candidate`, `fragments`, `valid`, `flanking_ltr`, `ltrdigest`, `ltrharvest`, `solo_ltr`, `ltr_retriever/`, `taxonomy`). `fragments/{genome}.gff3` is the unanchored (non-LTR) hit set; `fragments/{genome}.classified.{gff3,bed}` is the recovered, taxonomically-called subset. Only `valid` carries a `_reduced` variant (merged overlapping ranges); `original`/`candidate` are unreduced. The composite ERV "assembly" is no longer a separate `erv_like` track — it is now the genus-founded `taxonomy/{genome}.gff3` (one feature per LTR-element locus, labelled by genus call). `solo_ltr/{genome}.gff3` carries probe_labels propagated from valid ERVs (see `docs/solo_ltr.md`).
...
- `results/plots/provirus/` — the **provirus panel**: 21 PNGs from `plot2sort` (density, raincloud, query-coverage, bar, balloon, heatmap-probe×species, virus waffle, three Sankey variants, each split into `main` / `accessory` / `full` where applicable) + 18 middle-stage PNGs from `stage_plot_generator` (homology↔LTR concordance, structure, refinement funnel, multiplicity, plus **pre-reduction overlap/redundancy** — self-overlap degree, reciprocal-overlap fraction, reduction collapse fold, total-bp before/after — and **LTRharvest/LTRdigest interaction** — distance-to-nearest-retrotransposon, position-within-provirus metagene, probe×Pfam-domain overlap, per-feature breakdown, strand concordance, element length vs hits). Every plot stamps its range tier and reduced/non-reduced state in the subtitle. `results/plots/circle_plots/` (per-genome Circos-style PNG + PDF) and `results/plots/hotspot_pdfs/` sit alongside, unchanged.
- `results/plots/erv-like/` — the **ERV-like structural panel**: 6 PNGs from `erv_like_plot_generator`, read from the genus-founded taxonomy loci table (completeness, canonical vs rearranged gene order, gene combinations, locus length, main-gene count, genus × gene composition heatmap).
- **Taxonomic classification** — `data/taxonomy_reference/` holds the build-once reference (`retro_reference.{faa,csv}`, `taxonomy.tsv`, `erv_class.tsv`, `trees/`, `manifest.yaml`); gitignored, rebuilt via `make reference`. Per-genome outputs: `results/tables/taxonomy_classification/{genome}.loci.csv` (+ Parquet under `data/tables/`) — the per-locus genus-founded ERV assembly (`genus_call`, `rank`, `confidence`, `confidence_tag`, `n_blastx_hits`, `method`, `is_mosaic`, `mosaic_composition`, `erv_class`, `completeness`, `canonical_order`, per-gene calls, `probe_label_set` provenance, `source`); `{genome}.fragments.csv` — the recovered fragments tier (same schema, `source=fragment`, gated to classified-only); `{genome}.classification_counts.csv` + `{genome}.fragments_counts.csv` — blastx-stage loss counters; `results/tables/taxonomy_classification/classification_report.csv` — tidy counts by genus / confidence / method + mosaic + integration totals, split by tier; `results/tracks/taxonomy/{genome}.gff3` + `.bed` (IGV, colour-by-genus); `results/plots/taxonomy/` — 12 PNGs: the composition panel (genus composition, rank resolution, method mix, ERV-class composition, mosaic alluvial, confidence) plus the evidence/confidence/fragments panel (evidence_depth, confidence_density, confidence_vs_evidence, novel_structure, source_yield, genus_by_source), derived from the loci + fragments tables so they stay concordant. **Loss analysis** — `results/tables/loss_analysis/loss_analysis.csv` (per-genome per-stage attrition funnel) + `novel_candidates/{genome}.novel_candidates.csv` (loci with `n_blastx_hits == 0`) + `results/plots/loss/` — 5 PNGs (loss_funnel, step_retention heatmap, fragment_recovery yield, novel_burden, loss_waterfall).
- `results/tables/hotspots/` + `results/tracks/hotspots/` — per-window CSV/Parquet + provenance manifest, merged-region GFF3/BED, and `results/plots/hotspot_pdfs/` (Manhattan / karyotype / Q-Q / summary). Detection is a deterministic Negative-Binomial GLM (`hotspot_detector.R` + `hotspot_analysis/*`); see [configuration.md `## hotspot`](configuration.md).
