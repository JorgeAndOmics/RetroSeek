...
**Solo-LTR detection (LTR_retriever, superseded)** - `ltr_retriever_prefilter`, `ltr_retriever`, `solo_ltr_integrator`, `solo_ltr_detector` (aggregate). Runs LTR_retriever over LTRharvest output pre-filtered by `element_hits/{genome}.gff3`; propagates RetroSeek probe labels onto discovered solo LTRs. See [`docs/solo_ltr.md`](solo_ltr.md) for the full mechanism and [ADR-003](adr/ADR-003-ltr-retriever-pre-filter.md) for the pre-filter rationale.
...
- ADR-005: wrap LTR_retriever invocation in a Python runner script (`workflow/scripts/solo_ltr/run_ltr_retriever.py`) for testability + log capture + fail-loud behaviour.
