...
- `results/tables/<name>/` — user-facing **CSV** copy of every table group, each in its own subdirectory (no loose files): `ranges_analysis` (`{genome}.{final_loci,homology_loci,ltr_structure,reduction_multiplicity,counts,erv_like_loci,erv_like_members,provirus_overlap,ltr_interaction,probe_domain_overlap,reduction_coverage}.csv`), `overlap_matrix`, `segmented_species`, `probe_pairs`, `solo_intact_ratio`, `hotspots`, `probe_dict`, `full_genome_blast`.
...
- `results/plots/erv-like/` — the **ERV-like panel**: 9 PNGs from `erv_like_plot_generator` (probe-combination frequency, completeness, full/partial by virus, probe × virus composition heatmap, observed gene order, canonical-vs-rearranged, candidate length, constituent-locus count, inter-probe gap vs the `max_join_distance` cutoff).
