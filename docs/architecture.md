...
**Taxonomic classification** — `taxonomy_reference` + `taxonomy_reference_trees` (build-once reference: genus-comprehensive proteins via Entrez, NCBI taxonomy, curated `erv_class.tsv`, per-gene placement trees; `make reference` / `--build-reference`), then `taxonomy_classify` (per-locus genus calls) + `taxonomy_plot_generator` (`--classify`). Reclassifies each valid LTR-element locus from its own marker sequence — POL/GAG by phylogenetic placement, weighted-LCA otherwise — into a calibrated `genus_call` (rank, confidence, method, mosaic, ERV class). The per-locus loci table is the genus-founded ERV assembly; legacy probe `virus`/`label` are kept as detection provenance. See [configuration.md `## classification`](configuration.md), [`docs/taxonomy_classification/`](taxonomy_classification/), and [ADR-007](adr/ADR-007-taxonomic-classification.md).
...
- **Taxonomic classification** — `data/taxonomy_reference/` holds the build-once reference (`retro_reference.{faa,csv}`, `taxonomy.tsv`, `erv_class.tsv`, `trees/`, `manifest.yaml`); gitignored, rebuilt via `make reference`. Per-genome outputs: `results/tables/taxonomy_classification/{genome}.loci.csv` (+ Parquet under `data/tables/`) — the per-locus genus-founded ERV assembly (`genus_call`, `rank`, `confidence`, `method`, `is_mosaic`, `mosaic_composition`, `erv_class`, `completeness`, `canonical_order`, per-gene calls, `probe_label_set` provenance); `results/tracks/taxonomy/{genome}.gff3` + `.bed` (IGV, colour-by-genus); `results/plots/taxonomy/` — 5 PNGs (genus composition, rank resolution, method mix, ERV-class composition, mosaic alluvial), derived from the loci tables so they stay concordant.
