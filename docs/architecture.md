...
- `results/tracks/` — GFF3 per stage (`original`, `candidate`, `orphans`, `valid`, `flanking_ltr`, `ltrdigest`, `ltrharvest`, `solo_ltr`, `ltr_retriever/`, `taxonomy`). `orphans/{genome}.gff3` is the unanchored (non-LTR) hit set, carrying a synthetic `Parent=` from proximity clustering; `orphans/{genome}.classified.{gff3,bed}` is the recovered, taxonomically-called subset. Only `valid` carries a `_reduced` variant (merged overlapping ranges); `original`/`candidate` are unreduced. The composite ERV "assembly" is no longer a separate `erv_like` track — it is now the taxon-founded `taxonomy/{genome}.gff3` (one feature per LTR-element locus, labelled by `taxon_call`). `solo_ltr/{genome}.gff3` carries probe_labels propagated from valid ERVs (see `docs/solo_ltr.md`).
