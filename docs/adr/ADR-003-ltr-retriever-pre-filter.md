...
- **Performance win.** Smaller input -> faster LTR_retriever runs. On large mammalian genomes, the candidate pool is typically reduced by 10-50x.
...
- Performance bottleneck on large probe sets - unlikely; the pre-filter is O(candidates x valid_ranges_on_chrom), both usually < 10k.
