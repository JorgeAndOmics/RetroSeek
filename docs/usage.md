...
| `--solo-ltr-detection`      | `solo_ltr_detector`           | LTRharvest GFF3 + `valid_ranges.gff3` + `{genome}.loci.csv` | `solo_ltr/{genome}.gff3` + `solo_ltr/{genome}.solo_ltr.csv` + `solo_intact_ratio/{genome}.csv` + `all_species.csv` |
...
- **`ltr_retriever`** - LTR_retriever / solo-LTR knobs: `substitution_rate`, `min_ltr_similarity`, `threads_per_genome` (also RepeatMasker's `-pa`, the main runtime lever), `source_scn` (Coupling A toggle: `retroviral` | `full`), `group_by` (ratio grouping: `segment` | `taxon_call` | `none`), `nearest_locus_max_distance` (Coupling B fallback window). See [`docs/configuration.md`](configuration.md#ltr_retriever) for the full reference and [`docs/solo_ltr.md`](solo_ltr.md) for the mechanism.
