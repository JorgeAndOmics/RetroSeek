...
| `--placement-trees`        | `placement_trees`             | published `.jplace` + host tree          | per-genome heat-trees (SVG/Newick/Nexus), EDPL + LWR tables, `cophylogeny_summary.{tier}.{gene}.csv`, `erv_composition.{tier}.{gene}.newick`, `{tier}.{gene}.krd_matrix.csv` |
