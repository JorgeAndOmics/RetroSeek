...
| `orphan_merge_gap` | int ≥ 0 (bp) | `10000` | Orphans (non-LTR-associated hits) have no LTR element to group by, so their gene-hits are clustered by **spatial proximity** into single non-overlapping loci: hits whose gap ≤ this value collapse into one orphan locus (a synthetic `Parent`), then classified/assembled exactly like anchored proviruses (`structure_class`, `is_mosaic`, one authoritative `taxon_call`). Default `10000` (10 kb) ≈ the span of a full-length retroviral/ERV provirus (2 LTRs + gag–pro–pol–env, ~7–12 kb), so gene fragments of one degraded provirus cluster together. Raise to merge more aggressively; lower to keep orphans finer-grained. Orphan loci stay flagged `source=orphan` (proximity-*inferred*, vs anchored's LTR-*confirmed*). See ADR-010. |
