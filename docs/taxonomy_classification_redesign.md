...
`P`. Each probe `p ∈ P` is a tuple `(accession, gene(p), genus(p), virus(p))`. Detection produces,
...
```
genus(L_i) = genus( argmax_{h ∈ H_i} tiebreak_rank(h) ),  tiebreak_rank ≈ (bitscore, qcov, identity, −evalue, pos, label)
```
...
For each gene `g`, a versioned **reference package** `R_g = (S_g, A_g, T_g, τ_g, M_g)`:
...
- `T_g`: a maximum-likelihood reference tree on `A_g` with fitted model parameters (the placement
  scaffold), pinned.
- `τ_g`: taxonomy map, tip → {genus, subfamily, family, …}, enabling LCA backoff at any rank.
...
Given placement weights `{(e, LWR(e))}` and taxonomy `τ_g`:

1. Let `e* = argmax_e LWR(e)`.
2. **Confident genus:** if `LWR(e*) ≥ θ_high` and `e*` maps to a single genus → assign `genus(e*)`,
   confidence `= LWR(e*)`.
3. **LCA backoff:** else take the minimal edge set `E*` with cumulative LWR ≥ `θ_mass`; assign the
   **lowest common ancestor** rank of `τ_g(E*)` (e.g. subfamily or "Gammaretrovirus-like"),
   confidence `= Σ_{e∈E*} LWR(e)`.
4. **Abstention:** if even the LCA is above family level, or total placed mass `< θ_min` → assign
   `unclassified_retrovirus`.

Thresholds `(θ_high, θ_mass, θ_min)` are config-exposed with documented defaults; resolution is
deterministic given placement output (G5). This is the formal replacement for the
`aggregation.virus/label` strategies: `best` → `θ_high` rule; `strict`/"ambiguous" → LCA backoff with
...
- **Mosaic flag:** TRUE if ≥2 genes yield confident calls (`conf ≥ θ_high`) to **different** genera
