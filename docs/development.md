...
- **Python**: ruff (lint + format, including the Google-style docstring rules), mypy strict. Configured in [`pyproject.toml`](../pyproject.toml). Target version 3.10. Tests need no docstrings (their names state the behaviour), and `__init__` / dunder methods are documented through their class. `make test-py` enforces a coverage floor (`fail_under` in `pyproject.toml`) that only ever rises.
- **R**: lint via `lintr` (`make lint-r`, part of `make check`, configured in [`.lintr.R`](../.lintr.R): line length 88 like ruff, snake_case names with SNAKE_CASE file-level constants). Aligned assignment blocks and one-line guards (`if (is.null(x)) return(NULL)`) are house style, so `styler` is not run over the code: it would undo both.
- **Complexity**: at most 8 per function, in both languages. [Qlty](https://qlty.sh) measures the Python side (`qlty smells`, `qlty metrics --functions`, configured in [`.qlty/qlty.toml`](../.qlty/qlty.toml), which reuses the ruff settings above); lintr's `cyclocomp_linter` measures the R side. Every function in `workflow/scripts` meets it; a change must keep it that way.
